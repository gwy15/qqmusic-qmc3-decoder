use std::fs::File;
use std::io::{Read, Result, Write};

const BUFFER_SIZE: usize = 8192;

fn get_xor_mask(mut pos: usize) -> u8 {
    // Refer: https://github.com/MegrezZhu/qmcdump/blob/master/src/crypt.cpp
    static MASK: [u8; 256] = [
        , 0xC, 0x6C, 0xA5, 0x47, 0xF7, 0xF6, 0x00, 0x79, 0x4A,
        0x11,
    ];
    pos %= 0x7FFF;

    return MASK[(pos * pos + 80923) % 256];
}

fn decode(buffer: &mut [u8], offset: usize) {
    buffer
        .iter_mut()
        .enumerate()
        .for_each(|(i, b)| *b ^= get_xor_mask(offset + i));
}

pub fn decode_file(mut fin: File, mut fout: File) -> Result<()> {
    let mut offset = 0;
    let mut buffer = [0; BUFFER_SIZE];

    loop {
        let read_length = fin.read(&mut buffer)?;
        if read_length == 0 {
            break;
        }
        decode(&mut buffer[..read_length], offset);
        fout.write(&buffer)?;
        offset += read_length;
    }
    Ok(())
}
